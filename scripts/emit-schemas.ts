...
// declaring a rootDocument gain a root $ref so a whole-document validator has
...
/** One emitted file: the profile's documents under $defs, and the root
 * $ref only when the profile names a single document root. */
type EmittedSchema = {
  $schema: string;
  $id: string;
  title: string;
  $defs: Readonly<Record<string, TSchema>>;
  $ref?: string;
};
...
for (const entry of Object.values(PROFILES)) {
  ...
  const doc: EmittedSchema = {
    $schema: "https://json-schema.org/draft/2020-12/schema",
    $id: `https://github.com/DynamicalSystemsGroup/dsg-idl/schema/${file}`,
    title: entry.literal,
    $defs: entry.documents,
  };
  if (entry.rootDocument !== undefined) doc["$ref"] = `#/$defs/${entry.rootDocument}`;
